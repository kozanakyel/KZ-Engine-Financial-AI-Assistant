...
from ohlc_data_process import (
    calculate_dmi_rsi_mfi,
    fetch_data,
    analyze_ichimoku,
    analyze_supertrend,
...
class TradingAdvisor:
    ...
    def get_advice(symbol: str):
        ...
        rsi_query = (
            f"RSI,{rsi_14:.2f},MFI,{mfi_14:.2f},DMP,{dmp_14:.2f},"
            f" DMN,{dmn_14:.2f},ADX,{adx_14:.2f}"
        )

        trend_query = (
            f"Ichmouku trend,{trend_ich},Detected price,{base_price[0]:.2f},Supertrend,{trend_super},"
            f" cross for supertrend,{cross_super},Current Price,{df.iloc[-1]['Close']:.2f}"
        )

        rsi_prompt_template = TradingAdvisor.create_advice_prompt_template(
            rsi_examples, rsi_prefix
        )
        trend_prompt_template = TradingAdvisor.create_advice_prompt_template(
            trend_examples, trend_prefix
        )
        ...
        if cross_super:
            ...
            return openai(prompt_template.format(query=trend_query))
        else:
            rand = random.randint(0, 1)
            prompt_template = templates[rand]
            return openai(prompt_template.format(query=queries[rand]))
